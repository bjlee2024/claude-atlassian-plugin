...
| Jira + Confluence 복합 작업 | **Atlassian MCP** | 통합 인증, Jira 지원 |
...
2. **Jira 작업** -> `Atlassian MCP` (Jira 전용 도구 제공)
3. **Jira + Confluence 연계** -> `Atlassian MCP` 로 Jira 이슈 확인 후 `confluence-cli` 로 문서 작업
